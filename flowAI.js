...
const generateAPIResponse = async (incomingMessageDiv) => {
  ...
  try {
    // Append the validation instruction directly to the user's input
    const appendedUserMessage = `${userMessage}\n\nVerifica se il prompt riguarda fitness, salute o sport. Se non lo è, rispondi con 'Prompt Non Valido'. Se lo è, rispondi in modo appropriato.`;
    ...
    const apiRequestBody = {
      contents: [
        ...
        { role: "user", parts: [{ text: appendedUserMessage }] }
      ]
    };
    ...
    chatHistory.push({ role: "assistant", text: apiResponse }); // Add response to chat history
    localStorage.setItem("chat-history", JSON.stringify(chatHistory)); // Save chat history

    // Show the AI's response with a typing effect
    showTypingEffect(apiResponse, textElement, incomingMessageDiv);
  } catch (error) {
    isResponseGenerating = false;
    textElement.innerText = `Errore: ${error.message}`;
